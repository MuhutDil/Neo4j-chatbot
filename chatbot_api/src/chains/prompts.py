"""Prompt templates for Cypher query generation and QA response formatting.

This module defines LangChain PromptTemplate objects used for:
1. Generating Cypher queries from natural language questions
2. Converting Cypher query results into human-readable answers

The templates include detailed instructions, schema information, examples,
and constraints to ensure correct Cypher query generation.
"""
...
# Descriptions of the prompt templates
cypher_generation_prompt.__doc__ = """
Prompt template for generating Cypher queries from natural language questions.

This prompt instructs the LLM to:
- Generate valid Cypher queries for Neo4j graph databases
- Use only provided schema relationships and properties
- Follow specific formatting rules (no explanations, proper aliasing, etc.)
- Handle edge cases like division by zero and missing properties
- Use state abbreviations for filtering
- Include example queries for reference

Input variables:
    schema: The Neo4j database schema with relationship types and properties
    question: The natural language question to convert to Cypher
"""

qa_generation_prompt.__doc__ = """
Prompt template for converting Cypher query results to human-readable answers.

This prompt instructs the LLM to:
- Take Cypher query results and form a natural language response
- Treat query results as authoritative information
- Never doubt or correct the provided data
- Handle empty results by stating uncertainty
- Properly format lists of names (especially those with commas)
- Assume time durations are in days unless specified otherwise

Input variables:
    context: The results from a Neo4j Cypher query
